...
import com.example.EcommerceStore.service.impl.ScheduleService;
...
public class ScheduleController {
  ...
  public String showScheduleForm(Model model, HttpSession session) {
    ...
    List<com.example.EcommerceStore.entity.Address> addresses = new ArrayList<>();
    for (com.example.EcommerceStore.entity.Address a : addressRepository.findByUser(userId)) {
  ...
  public String processScheduleForm(@RequestParam("orderDetail") long orderDetailId,
    ...
    schedule.setTime(Date.valueOf(String.valueOf(sqlDate))); // Thời gian lịch được chọn
    schedule.setStatus("PROCESSING");
    schedule.setPhone(phone);
    schedule.setName(name);
    schedule.setShift(shift);
    schedule.setUser(userRepository.findUserByUserId((int) session.getAttribute("user_id")));
